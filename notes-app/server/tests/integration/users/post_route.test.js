...
describe("the Users POST route", () => {
  ...
  test("a new user can be added", async () => {
    ...
    // Remove the id, notes and timestamps fields from the returned object
  ...
  test("a newly created user should have no notes", async () => {
    const user = initialUsers[0];
    ...
    // Assert the user has no notes
    assert(getResponse.body.notes, []);
  });
  ...
  test("the username cannot be below the min value length", async () => {
    ...
    assert.ok(
      response.body.error.includes(
        `The username must be between ${minUsernameLength} and ${maxUsernameLength} chars long`,
      ),
    );
  ...
  test("the name cannot be below the min value length", async () => {
    ...
    assert.ok(
      response.body.error.includes(
        `The user's name must be between ${minNameLength} and ${maxNameLength} chars long`,
      ),
    );
  ...
  test("the password cannot be below the min value length", async () => {
    ...
    assert.ok(
      response.body.error.includes(
        `Password length must be between ${minPasswordLength} and ${maxPasswordLength} chars long`,
      ),
    );
